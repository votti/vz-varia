...
However, I also would highly recommend looking into Datashader when using Python.
...
```{r}
p <- ggplot(dists_shuffled, aes(x=x, y=y))+
    stat_bin2d(aes(color=..count..), bins=100, geom='point', size=1, fill=1)+
    scale_color_viridis(trans = "log10", option='inferno')+
    ggtitle('Bin2D Points\nCoarser Fake Points\nRuntime: ca 0.5 s')
...
```{r}
p <- ggplot(dists, aes(x=x, y=y))+
    stat_summary_2d(aes(z=value, color=stat(value)),bins=500,fun=get_first, geom='point', fill=1, size=1/20)+
...
When using the shuffeled version, this can be (slightly faster) achieved by just taking the first element every time. This avoids sampling.
